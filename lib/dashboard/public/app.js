...
// The 11 native tab ids, mirroring ops.js's NATIVE_TABS exactly — the client's own source of truth
// for "what's a real native tab id" (used to validate P.config.navTabs before trusting it, same
// defensive stance as ROUTES further down). Future tasks (Bloc note, Daily meeting) register their
// own id into this same list — this task only builds the mechanism, no placeholder ids added here.
const NATIVE_TABS = ['board', 'chat', 'requests', 'attention', 'backlog', 'workflow', 'team', 'files', 'info', 'docs', 'personalize'];
function defaultNavTabs() { return NATIVE_TABS.map((id) => ({ id, enabled: true })); }
...
function currentNavTabs() {
  ...
  const valid = Array.isArray(arr) && arr.length === NATIVE_TABS.length
    && arr.every((v) => v && typeof v.id === 'string' && NATIVE_TABS.includes(v.id))
    && new Set(arr.map((v) => v.id)).size === NATIVE_TABS.length;
  const list = valid ? arr : defaultNavTabs();
  return list.map((v) => ({ id: v.id, enabled: v.id === 'personalize' ? true : !!v.enabled }));
}
