...
// The 12 native tab ids, mirroring ops.js's NATIVE_TABS exactly — the client's own source of truth
...
// 2 (Sous-projet C) registers 'notes' — the Bloc note scratchpad — as the first tab that must start
// OFF by default (an opt-in feature, per the user's explicit request), so DEFAULT_OFF_TABS exists
// rather than a blanket "every native tab defaults to enabled" the way Task 1 assumed.
const NATIVE_TABS = ['board', 'chat', 'requests', 'attention', 'backlog', 'workflow', 'team', 'files', 'notes', 'info', 'docs', 'personalize'];
const DEFAULT_OFF_TABS = new Set(['notes']);
...
function render(){
  ...
  renderSidebar(); renderRequests(); renderAttention(); renderInfo(); renderDocs(); renderSettings(); renderFiles(); renderNotes(); applySideHidden(); updateChatBusyUI();
...
const ROUTES=['board','requests','attention','backlog','workflow','team','files','notes','chat','info','docs','personalize'];
