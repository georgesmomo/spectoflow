...
// Interim: lands one level under a `dashboard` subfolder of that root — this is the seam Task 6
// replaces with a global-config-driven workspace path; kept here now so the re-based tests already
// exercise a baseDir that isn't the workspace root itself.
function registryDir(baseDir) {
  return baseDir || path.join(process.env.SPECTOFLOW_HOME || path.join(os.homedir(), '.spectoflow'), 'dashboard');
}
